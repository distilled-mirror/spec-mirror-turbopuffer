...
By default, the destination also inherits the source's
[sharding](#param-sharding) configuration. Set `sharding` on the copy request to
...
The client libraries expose a `copy_from()` / `copyFrom()` convenience method
that wraps this parameter.
...
See [Copy from a namespace](#copy-from-a-namespace) for a detailed example.
...
The branch inherits the source namespace's
[`read_only`](/docs/metadata#change-metadata) setting when it is created. It
can be changed independently afterwards.
...
configure full-text search for an attribute, disable filtering for an attribute,
or enable [native embedding](/docs/embedding).
...
Partitions the namespace across multiple internal shards to scale beyond the
size limit of a single index. Can be set on the namespace's first write, or with
[`copy_from_namespace`](#param-copy_from_namespace). It cannot be changed on an
existing namespace in place.

On `copy_from_namespace`, omit `sharding` to inherit the source configuration.
Set a different `num_shards` to create a resharded copy.

See the [sharding guide](/docs/sharding) for details on when and how to use
sharding.

Contains the following fields:

- `num_shards` (integer): The number of shards to partition the namespace
  across, between 1 and 256.
...
Currently, turbopuffer does not re-encrypt data when you rotate key versions, meaning old data will remain encrypted using older key versions, while fresh writes will be encrypted using the latest versions.
...
Disables HTTP 429 backpressure on writes when unindexed data exceeds 2 GB or 1 million rows, whichever comes first. Useful for initial data loading or bulk updates. When disabled, strongly consistent queries return errors above these thresholds, so use [eventual consistency](/docs/query#param-consistency) instead. Eventually consistent queries search only the first 128 MiB of unindexed data.

These thresholds are measured per namespace, not per shard. A sharded namespace
...
- `[][N]f32`: Variable-length array of `N` dimensional f32 vectors, used for [late-interaction (multi-vector) search](/docs/query#late-interaction). Example: `[[0.1, 0.2], [0.3, 0.4]]`.
...
On a vector array attribute (`[][N]f32`), `ann: {"late_interaction": true}` builds the index used for [late-interaction (multi-vector) search](/docs/query#late-interaction). Set `ann: false` to store the per-token vectors without building the index (exact [`kNN`](/docs/query#knn-exact-search) queries still work, but not `ANN` queries).
...
Configure [native embedding](/docs/embedding) for this field. Can only be set
on `string` fields.

Can either be the [name of an embedding model](/docs/embedding#models)
or an object with the following fields:

- `model` (string): The model to use to embed this field. This field is
  required.

- `attribute` (string): The vector attribute that stores embedding vectors for
  this field. If omitted, turbopuffer creates a computed vector attribute named
  `embed_<attribute>`. The vector must use a dimensionality and data type
  supported by the `model`. Required when enabling embedding on an existing
  attribute.

- `dims` (int): The number of output dimensions to embed at. If not set, will
  use the model's default dimensionality. Must be set to a dimension supported
  by the model. Must match the size of the vector specified in `attribute`.

- `dtype` (string): The data type of the embedding vectors. Must match the type
  of the vector specified in `attribute`, and must be a data type supported by
  the model. If not set, will be inferred from the target attribute's type or
  the model's default data type.
...
- `glob`
- `fuzzy`
- `embed` (can be added or removed, but not modified)
...
and upserting into a new namespace if you require a schema change.
    ...
    After enabling the `filterable`, `full_text_search`, `regex`, `glob`, or `fuzzy` setting for an existing attribute, the index needs time to build before queries that depend on the index can be executed. turbopuffer will respond with HTTP status 409 to queries that depend on an index that is not yet built.
...
```bash
# choose best region: https://turbopuffer.com/docs/regions
curl https://gcp-us-central1.turbopuffer.com/v2/namespaces/write-copy-from-dest-curl \
  -X POST --fail-with-body \
  -H "Authorization: Bearer $TURBOPUFFER_API_KEY" \
  -H 'Content-Type: application/json' \
  -d '{
  "copy_from_namespace": "write-copy-from-source-curl-'"$NONCE"'"
}'

# To copy across regions, organizations, or cloud providers, pass an object
# instead of a string:
# {
#   "copy_from_namespace": {
#     "source_namespace": "write-copy-from-source-curl-'"$NONCE"'",
#     "source_region": "aws-us-east-1",
#     "source_api_key": "tpuf_A1..."
#   }
# }
...
destination = tpuf.namespace(f'write-copy-from-dest-py')
destination.copy_from(
    source_namespace=f'write-copy-from-source-py',
    # To copy across regions, organizations, or cloud providers, also pass:
    # source_region='aws-us-east-1',
    # source_api_key='tpuf_A1...',
)
```
...
const destination = tpuf.namespace(`write-copy-from-dest-ts`);
await destination.copyFrom({
  source_namespace: `write-copy-from-source-ts`,
  // To copy across regions, organizations, or cloud providers, also pass:
  // source_region: "aws-us-east-1",
  // source_api_key: "tpuf_A1...",
});
...
func main() {
	...
	destination := tpuf.Namespace("write-copy-from-dest-go")
	if _, err := (&destination).CopyFrom(ctx, turbopuffer.NamespaceCopyFromParams{
		SourceNamespace: "write-copy-from-source-go",
		// To copy across regions, organizations, or cloud providers, also set:
		// SourceRegion: turbopuffer.String("aws-us-east-1"),
		// SourceApiKey: turbopuffer.String("tpuf_A1..."),
	}); err != nil {
		panic(err)
	}
}
...
public class WriteCopyFrom {
  ...
  public static void main(String[] args) {
    ...
    var destination = tpuf.namespace("write-copy-from-dest-java");
    destination.copyFrom(
      NamespaceCopyFromParams.builder()
        .sourceNamespace("write-copy-from-source-java")
        // To copy across regions, organizations, or cloud providers, also set:
        // .sourceRegion("aws-us-east-1")
        // .sourceApiKey("tpuf_A1...")
        .build()
    );
  }
}
...
var destination = tpuf.Namespace("write-copy-from-dest-csharp");
await destination.CopyFrom(
    new NamespaceCopyFromParams
    {
        SourceNamespace = "write-copy-from-source-csharp",
        // To copy across regions, organizations, or cloud providers, also set:
        // SourceRegion = "aws-us-east-1",
        // SourceApiKey = "tpuf_A1...",
    }
...
destination = tpuf.namespace("write-copy-from-dest-rb")
destination.copy_from(
  source_namespace: "write-copy-from-source-rb",
  # To copy across regions, organizations, or cloud providers, also pass:
  # source_region: "aws-us-east-1",
  # source_api_key: "tpuf_A1...",
)
